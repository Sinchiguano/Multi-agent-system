...
def main():
    ...
    while (True):
        ...
        # X=goal_position.x+l_radio*math.sin(current_position.z)
        # Y=goal_position.y-l_radio*math.cos(current_position.z)
        # alpha=k1/(k2+euclidean_distance(X,Y))

        tmp=goal_position.x*math.sin(current_position.z)-goal_position.y*math.cos(current_position.z)
        tmp1=tmp
        alpha=k1/(k2*+sqrt((goal_position.x**2)+(goal_position.y**2)+(l_radio**2)+2*l_radio*tmp))

        #Limit the linear velocity
        aux=tmp1+l_radio
        u1=alpha*aux
        #u1=alpha*(goal_position.x*math.sin(current_position.z)-goal_position.y*math.cos(current_position.z)+l_radio)
        print('velocity:',u1)
        if (u1>0.3):
            u1=0.3
        elif (u1<-0.3):
        ...
        #u2=(-1/l_radio)*alpha*(goal_position.x*math.sin(current_position.z)+goal_position.y*math.cos(current_position.z))
        u2=(-alpha/l_radio)*(goal_position.x*math.cos(current_position.z)+goal_position.y*math.sin(current_position.z))
        if (u2>0.3):
            u2=0.3

        #Linear velocity
        velo_cmd_1.linear.x =u1
        print('velocity:',u1)
