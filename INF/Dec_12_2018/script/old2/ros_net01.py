...
def main():
    ...
    #while (True):
        ...
        try:
            ...
            now = rospy.Time.now()
            listener_r2.waitForTransform('/robot2/base_footprint','/robot1/base_footprint', now, rospy.Duration(5.0))
            trans,rot= listener_r2.lookupTransform('/robot2/base_footprint','/robot1/base_footprint', now)
        except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
            continue
        # try:
        #     now = rospy.Time.now()
        #     past = now - rospy.Duration(5)
        #     listener_r2.waitForTransformFull('robot2_tf/base_footprint',now,'robot1_tf/base_footprint', now,
        #                               'world', rospy.Duration(5.0))
        #     (trans, rot) = listener_r2.lookupTransformFull('robot2_tf/base_footprint', now,
        #                               'robot1_tf/base_footprint', now,
        #                               'world')
        # except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
        #     continue
        print(trans)
